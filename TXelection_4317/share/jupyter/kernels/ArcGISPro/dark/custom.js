(function() {
  ...
  document.addEventListener('DOMNodeInserted', function() {
	var divs = document.querySelectorAll('#messages');
	divs.forEach( div => {
		if(div.dataset.show === "true") {
			div.dataset.show = false;
			let messages = JSON.parse(div.dataset.messages)
			Details(messages, div);
		}
	});
  });
  
  $.getScript("/custom/messages.js");
  $('head').append( $('<link rel="stylesheet" type="text/css" />').attr('href', '/custom/messages.css') );
